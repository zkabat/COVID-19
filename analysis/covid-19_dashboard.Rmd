...
Top countries according to new cases (deaths) on `r latest_date`.

```{r}
renderPlotly({
  if (input$metric2 == "Total cases") {
    data_trend() %>%
      filter(country %in% top_country_range(., 1, input$nlines, cases_new)) %>%
    ...
    data_trend() %>%
      filter(country %in% top_country_range(., 1, input$nlines, deaths_new)) %>%
...
Top countries according to new cases (deaths) per million on `r latest_date`.

```{r}
renderPlotly({
  if (input$metric2 == "Total cases") {
    data_trend() %>%
      filter(country %in% top_country_range(., 1, input$nlines, cases_new_per_mil)) %>%
    ...
    data_trend() %>%
      filter(country %in% top_country_range(., 1, input$nlines, deaths_new_per_mil)) %>%
