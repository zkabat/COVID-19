...
g <- data_sum %>%
  ...
  geom_point(color = "navy") + geom_smooth() + theme_light()
...
selectInput("subset", label = "Select subset:",
            choices = c("Top cummulative", "Top new", "Top growth"),
            selected = "Top cummulative")

sliderInput("maxdays", label = "Maximum days to plot:",
            min = 10, max = 100, value = 60, step = 10)
...
data_trend_cases <- reactive({
  if(input$subset == "Top cummulative") {
    data_trend() %>% 
      filter(country %in% top_countries(., input$nlines, cases_cum))
  } else if (input$subset == "Top new") {
    data_trend() %>%
      filter(country %in% top_countries(., input$nlines, cases_new))
  } else if (input$subset == "Top growth") {
    data_trend() %>% 
      filter(country %in% top_countries(., input$nlines, 
                                        cases_3day_growth_rate_pct))
  }
})
...
data_trend_deaths <- reactive({
  if(input$subset == "Top cummulative") {
    data_trend() %>% 
      filter(country %in% top_countries(., input$nlines, deaths_cum))
  } else if (input$subset == "Top new") {
    data_trend() %>%
      filter(country %in% top_countries(., input$nlines, deaths_new))
  } else if (input$subset == "Top growth") {
    data_trend() %>% 
      filter(country %in% top_countries(., input$nlines, 
                                        deaths_3day_growth_rate_pct))
  }
})
```
