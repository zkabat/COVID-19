...
data <- data %>% 
  mutate(cases_cum_per_100k = round(cases_cum/population*100000, 3),
         ...
         recovered_cum_per_100k = round(recovered_cum/population*100000, 6),
         recovered_new_per_100k = round(recovered_new/population*100000, 6),
         recovered_new_smooth_per_100k = 
           round(recovered_new_smooth/population*100000, 6))
...
data %>%
  ...
  select(country, recovered_cum, recovered_new,
         recovered_cum_per_100k, recovered_new_per_100k) %>%
  arrange(desc(recovered_new)) %>%
  datatable(colnames = c("Country", "Total recovered", "New recovered",
                         "Total recovered per 100k", "New recovered per 100k"),
            rownames = FALSE,
            options = list(pageLength = 15))
```
...
data %>%
  filter(cases_cum >= CASE_CUTOFF) %>%
  plot_data(latest_date, recovered_cum_per_100k, region)
```
...
data %>%
  filter(cases_cum >= CASE_CUTOFF) %>%
  plot_data(latest_date, recovered_new_per_100k, region)
```
